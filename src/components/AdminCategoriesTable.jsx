...
useEffect(() => {
  ...
  const handleUpdate = async ({ _id, values }) => {
    ...
    if (updated) {
      setTableData((prev) => {
        if (_id) {
          return prev.map((r) => (r._id === _id ? updated : r));
        } else {
          return [...prev, updated];
        }
      });
      ...
      <table className="w-full border-collapse">
        <thead>
          <tr className="bg-brown border-b border-gray-200 text-xl text-left text-white">
            ...
            <th className="p-5 font-semibold">Sub Title</th>
                    ...
                    onClick={async () => {
                      if (navigator.share) {
                        await navigator.share({
                          title: "Admin Categories Data",
                          text: "Check out these categories",
        ...
        <tbody>
          {currentData.map((row, idx) => (
            ...
            >
              ...
              <td className="p-5 border-b border-gray-200">
                <div>
                  <p className="font-medium">{row.caption}</p>
                  <p className="text-sm text-gray-500">{row.name}</p>
                </div>
              </td>
              <td className="p-5 border-b border-gray-200">{row.sub_title}</td>
              <td className="p-5 border-b border-gray-200">
                <span
      ...
      {modal.row && (
        <AdminDataUpdate
          ...
          formFields={[
            ...
            { name: "caption", label: "Category Caption", type: "text" },
            ...
            { name: "sub_title", label: "Sub Title", type: "textarea" },
